...
from ..repositories.sqlalchemy_project import SqlAlchemyProjectRepository
from ..repositories.sqlalchemy_task import SqlAlchemyTaskRepository
from ..core.models import Project, Task  # core dataclasses
...
def _deps() -> Tuple[
    ...
    """
    Open a DB session, create SQLAlchemy repositories and services,
    and yield them for use within a single CLI action.
    """
    with session_scope() as session:
        ...
        project_service = ProjectService(
            project_repo,
            cascade_delete_tasks=task_repo.delete_by_project,
        )
        task_service = TaskService(
            project_repo=project_repo,
            task_repo=task_repo,
        )
        yield project_repo, task_repo, project_service, task_service
...
def action_create_project() -> None:
    ...
    with _deps() as (_, __, service, _task_service):
        try:
            p: Project = service.create_project(name, desc)
...
def action_list_projects() -> None:
    ...
    with _deps() as (project_repo, _, service, _task_service):
        if project_repo.count() == 0:
            print("(no projects yet)")
        else:
            for p in sorted(project_repo.all(), key=lambda x: x.created_at):
...
def action_info() -> None:
    ...
    with _deps() as (project_repo, _, service, _task_service):
        ...
        print(f"Current count         = {project_repo.count()}")
...
def action_edit_project() -> None:
    ...
    with _deps() as (project_repo, _, service, _task_service):
        current = project_repo.get_by_id(pid)
        if not current:
        ...
        try:
            p = service.update_project(pid, name=new_name, description=new_desc)
...
def action_add_task() -> None:
    ...
    with _deps() as (_, __, _service, task_service):
        try:
            t: Task = task_service.create_task(
                project_id=pid,
                title=title,
                description=desc,
                status=status,
            ...
            print(
                f"\n[ok] Task created: id={t.id} project=#{t.project_id} "
                f"status={t.status} deadline={shown_deadline}"
            )
...
def action_delete_project() -> None:
    ...
    pid = int(pid_str)
    confirm = input(
        f"Are you sure you want to delete project #{pid}? "
        f"This will also delete its tasks. [y/N]: "
    ).strip().lower()
...
def action_change_task_status() -> None:
    ...
    with _deps() as (_, __, _service, task_service):
        try:
            t = task_service.set_task_status(tid, status)
...
def action_edit_task() -> None:
    ...
    with _deps() as (_, task_repo, _service, task_service):
        # Fetch current task to show values
        t = task_repo.get_by_id(tid)
        if not t:
            print(f"\n[error] task #{tid} not found")
            return _pause()
        ...
        try:
            t2 = task_service.update_task(tid, **kwargs)
            d_show = t2.deadline.isoformat() if t2.deadline else "—"
            print(
                f"\n[ok] updated task #{t2.id}  status={t2.status}  "
                f"deadline={d_show}"
            )
        except (TaskNotFound, ValidationError) as e:
            print(f"\n[error] {e}")
...
def action_delete_task() -> None:
    ...
    confirm = input(
        f"Delete task #{tid} in project #{pid}? [y/N]: "
    ).strip().lower()
    if confirm not in {"y", "yes"}:
        print("\n[cancelled] no changes made.")
        return _pause()
...
def action_list_project_tasks() -> None:
    ...
    with _deps() as (_, __, _service, task_service):
        try:
            tasks = task_service.list_tasks_for_project(pid)
        except ProjectNotFound as e:
            print(f"\n[error] {e}")
            return _pause()
...
# Note: we do NOT have "reset memory" here, because DB is persistent.
